...
public class Client {
    ...
    try{
        ...
        byte buff3[] = String.valueOf(z).getBytes();
        DatagramPacket z_value = new DatagramPacket(buff3,buff3.length,addsv,1234);
        cl.send(z_value);     
        
        //Received server Result
        System.out.println("Receiving LCM value from server:");
        byte buff4[] = new byte[256];
        DatagramPacket data_result = new DatagramPacket(buff4,buff4.length);
        cl.receive(data_result);
        String received_data = new String(data_result.getData(),0,data_result.getLength());
        System.out.println("Result From Server(LCM):"+received_data);
        cl.close();
        } 
        catch(IOException e){
